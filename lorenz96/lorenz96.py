...
from utils import get_lorenz96, compute_nrmse, plot_train_test_prediction_and_target, plot_prediction_distribution, plot_error, plot_prediction_3d
import numpy as np
import argparse
...
for guess in range(config["test_trials"]):
    ...
    plot_train_test_prediction_and_target(train_predictions, train_target, test_predictions, test_target, inp_dim=len(predicted_dims), labels=[f"x{i}" for i in predicted_dims]) if config["show_plot"] else None
    plot_error(test_predictions, test_target, n_dim=len(predicted_dims), labels=[f"Error in x{i}" for i in predicted_dims]) if config["show_plot"] else None
    plot_prediction_distribution(train_predictions, train_target, "Train", labels=[f"x{i}" for i in predicted_dims]) if config["show_plot"] else None
    plot_prediction_distribution(test_predictions, test_target, "Test", labels=[f"x{i}" for i in predicted_dims]) if config["show_plot"] else None
    plot_prediction_3d(test_predictions, test_target, title="Lorenz96", labels=[f"x{i}" for i in predicted_dims]) if config["show_plot"] and len(predicted_dims) == 3 else None

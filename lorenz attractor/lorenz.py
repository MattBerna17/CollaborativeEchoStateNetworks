...
for guess in range(config["test_trials"]):
    ...
    if model.mode == "URP":
        ...
        elif model.mode == "CDR" or model.mode == "EMP-3":
    ...
    if model.mode == "URP":
        test_predictions = model.predict(n, Y=test_target) # get the model's prediction for n iterations
    else:
    ...
    n_var = 2 if model.mode == "ZCDR" or model.mode == "CDR" else 3
    test_target = test_target[:, 0:n_var].numpy()
    train_target = train_target[:, 0:n_var]
    NRMSE = [compute_nrmse(test_predictions, test_target)] # compute nrmse for each prediction

    plot_train_test_prediction_and_target(train_predictions, train_target, test_predictions, test_target, inp_dim=n_var) if config["show_plot"] else None
